...
const scratchJsPath = path.join(rootDir, 'scratch-card.js');
...
test('File Existence & Readability', async (t) => {
  await t.test('Core files exist and are non-empty', () => {
    ...
    assert.ok(fs.existsSync(scratchJsPath), 'scratch-card.js exists');
    assert.ok(fs.statSync(scratchJsPath).size > 0, 'scratch-card.js is non-empty');
  });
});
...
test('HTML Markup & Accessibility Audits', async (t) => {
  ...
  await t.test('Contains semantic landmarks and required interactive DOM IDs', () => {
    const requiredIds = ['ticket3dScene', 'ticketCard', 'cinemaToast', 'heroStage'];
...
test('CSS Styling & Asset Verification', async (t) => {
  ...
  await t.test('CSS curly braces are balanced and valid', () => {
    let openCount = 0;
    let closeCount = 0;
    ...
    assert.equal(openCount, closeCount, `CSS has mismatched braces: ${openCount} { vs ${closeCount} }`);
  });
...
test('JavaScript File Syntax & Integrity', async (t) => {
  ...
  const scratchJs = fs.readFileSync(scratchJsPath, 'utf8');

  await t.test('Contains event listeners and physics hooks in script.js', () => {
  ...
  await t.test('Contains scratch card canvas, pointer events and a11y handlers in scratch-card.js', () => {
    assert.ok(scratchJs.includes('destination-out'), 'Uses destination-out composite operation');
    assert.ok(scratchJs.includes('pointermove'), 'Listens to pointermove events');
    assert.ok(scratchJs.includes('pointerdown'), 'Listens to pointerdown events');
    assert.ok(scratchJs.includes('scratch-a11y-btn'), 'Contains accessible reveal button');
    assert.ok(scratchJs.includes('prefers-reduced-motion'), 'Supports reduced motion preference');
  });
});
...
test('Local Server HTTP Serving & MIME Type Test', async (t) => {
  ...
  try {
    ...
    await t.test('Serves scratch-card.js with 200 OK', async () => {
      const res = await fetch(`${baseUrl}/scratch-card.js`);
      assert.equal(res.status, 200);
      assert.ok(res.headers.get('content-type')?.includes('text/javascript'));
    });
